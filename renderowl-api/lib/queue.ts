...
export class JobQueue extends EventEmitter {
  ...
  async enqueue<T>(
    ...
    try {
      ...
      if (error.message?.includes('UNIQUE constraint failed: jobs.idempotency_key')) {
        ...
        ).get(options.idempotencyKey) as any;
        ...
        if (existing) {
          ...
          return this.hydrateJob(existing) as Job<T>;
        }
      }
      throw error;
    }
  ...
  async start(pollIntervalMs = 1000): Promise<void> {
    ...
    this.stalledCheckInterval = setInterval(
      ...
      this.retryConfig.stalledCheckIntervalMs
    );
  ...
  getJobByIdempotencyKey(key: string): Job | null {
    const row = this.db.prepare('SELECT * FROM jobs WHERE idempotency_key = ?').get(key) as any;
    return row ? this.hydrateJob(row) : null;
  }
  ...
  async retryDeadLetter(dlqId: string): Promise<Job | null> {
    const dlqJob = this.db.prepare('SELECT * FROM dead_letter_jobs WHERE id = ?').get(dlqId) as any;
