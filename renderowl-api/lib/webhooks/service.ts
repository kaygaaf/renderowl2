...
export class WebhookService extends EventEmitter {
  ...
  getWebhook(id: string, includeSecret = false): WebhookEndpoint | null {
  ...
  getWebhooksByUser(userId: string, includeSecret = false): WebhookEndpoint[] {
    const rows = this.db.prepare('SELECT * FROM webhook_endpoints WHERE user_id = ? ORDER BY created_at DESC').all(userId) as any[];
  ...
  updateWebhook(
    id: string,
    updates: Partial<Pick<WebhookEndpoint, 'url' | 'events' | 'status' | 'description' | 'headers' | 'maxRetries'>>
  ...
  private hydrateWebhook(row: any, includeSecret: boolean): WebhookEndpoint {
    return {
      id: row.id,
      userId: row.user_id,
      url: row.url,
  ...
  private hydrateDelivery(row: any): WebhookDelivery {
    return {
      id: row.id,
