...
export class BatchJobService extends EventEmitter {
    ...
    createBatchJob(params) {
        ...
        this.db.prepare(`INSERT INTO batch_jobs (
        id, user_id, project_id, template_id, name, operations, total_jobs, priority, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run([job.id, job.userId, job.projectId, job.templateId, job.name,
            JSON.stringify(job.operations), job.totalJobs, job.priority, job.createdAt]);
        // Create individual render job records
        const insertRenderJob = this.db.prepare(`INSERT INTO batch_render_jobs (id, batch_job_id, operation_index, variables)
       VALUES (?, ?, ?, ?)`);
    ...
    // Job Retrieval
    // --------------------------------------------------------------------------
    getBatchJob(jobId, userId) {
        const row = this.db.prepare(`SELECT * FROM batch_jobs WHERE id = ? AND user_id = ?`).get(jobId, userId);
        if (!row)
            return null;
        return this.rowToBatchJob(row);
    }
    getBatchJobs(userId, options) {
    ...
    getBatchRenderJobs(batchJobId) {
        const rows = this.db.prepare(`SELECT * FROM batch_render_jobs WHERE batch_job_id = ? ORDER BY operation_index`).all(batchJobId);
        return rows.map(r => ({
            id: r.id,
    ...
    // Job Control
    // --------------------------------------------------------------------------
    pauseBatchJob(jobId, userId) {
        const result = this.db.prepare(`UPDATE batch_jobs 
       SET status = 'paused', updated_at = datetime('now')
       WHERE id = ? AND user_id = ? AND status = 'processing'`).run([jobId, userId]);
        if (result.changes > 0) {
    ...
    resumeBatchJob(jobId, userId) {
        const result = this.db.prepare(`UPDATE batch_jobs 
       SET status = 'pending', updated_at = datetime('now')
       WHERE id = ? AND user_id = ? AND status = 'paused'`).run([jobId, userId]);
        if (result.changes > 0) {
    ...
    cancelBatchJob(jobId, userId) {
       ...
       WHERE id = ? AND user_id = ? AND status IN ('pending', 'processing', 'paused')`).run([jobId, userId]);
        if (result.changes > 0) {
            // Cancel any pending render jobs
            this.db.prepare(`UPDATE batch_render_jobs 
         SET status = 'cancelled', completed_at = datetime('now')
         WHERE batch_job_id = ? AND status IN ('pending', 'queued', 'processing')`).run([jobId]);
            this.emit('jobCancelled', { jobId });
            return true;
        }
        return false;
    }
    deleteBatchJob(jobId, userId) {
        // This will cascade delete batch_render_jobs
        const result = this.db.prepare(`DELETE FROM batch_jobs WHERE id = ? AND user_id = ?`).run([jobId, userId]);
    ...
    async processPendingJobs() {
        ...
        try {
            ...
            const pendingJobs = this.db.prepare(`SELECT * FROM batch_jobs 
    ...
    async processBatchJob(jobRow) {
       ...
       LIMIT 5`).all(jobId);
        if (pendingRenderJobs.length === 0) {
         ...
         FROM batch_render_jobs WHERE batch_job_id = ?`).get(jobId);
           ...
           WHERE id = ?`).run([stats.completed, stats.failed, progress, jobId]);
        ...
        // Update progress
        const progressStats = this.db.prepare(`SELECT 
        COUNT(*) as total,
        SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed,
        SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) as failed
       FROM batch_render_jobs WHERE batch_job_id = ?`).get(jobId);
       ...
       WHERE id = ?`).run([progressStats.completed, progressStats.failed, progress, jobId]);
        this.emit('jobProgress', { jobId, progress, completed: progressStats.completed, failed: progressStats.failed });
    }
    async processRenderJob(renderJob, batchJob) {
        try {
            // Mark as queued
            this.db.prepare(`UPDATE batch_render_jobs SET status = 'queued' WHERE id = ?`).run([renderJob.id]);
         ...
         WHERE id = ?`).run([error.message || 'Unknown error', renderJob.id]);
        }
    }
    // --------------------------------------------------------------------------
    // Render Job Updates
    ...
    getBatchStats(userId) {
       ...
       FROM batch_jobs WHERE user_id = ?`).get(userId);
        return {
            total: stats.total || 0,
...
// Route Handlers
// ============================================================================
export default async function batchJobRoutes(fastify, opts) {
    ...
    // --------------------------------------------------------------------------
    fastify.post('/from-csv', async (request, reply) => {
        const userId = request.user?.id;
        if (!userId) {
            return reply.status(401).send({ error: 'Unauthorized' });
        }
