...
export class ApiKeyService extends EventEmitter {
    ...
    `).run(apiKey.id, apiKey.userId, apiKey.name, apiKey.keyHash, apiKey.keyPreview, JSON.stringify(apiKey.scopes), apiKey.status, apiKey.expiresAt, apiKey.allowedIps ? JSON.stringify(apiKey.allowedIps) : null, apiKey.allowedOrigins ? JSON.stringify(apiKey.allowedOrigins) : null, apiKey.metadata ? JSON.stringify(apiKey.metadata) : null, apiKey.createdAt);
     ...
     */
    recordUsage(apiKeyId, endpoint, method, ipAddress, userAgent, success, errorMessage) {
    ...
    getUserApiKeys(userId) {
        const rows = this.db.prepare('SELECT * FROM api_keys WHERE user_id = ? ORDER BY created_at DESC').all(userId);
     ...
     */
    getUsageStats(apiKeyId, days = 30) {
        const since = Math.floor(Date.now() / 1000) - days * 86400;
        const total = this.db.prepare('SELECT COUNT(*) as count FROM api_key_usage WHERE api_key_id = ? AND timestamp > ?').get(apiKeyId, since);
        const successful = this.db.prepare('SELECT COUNT(*) as count FROM api_key_usage WHERE api_key_id = ? AND timestamp > ? AND success = 1').get(apiKeyId, since);
        const failed = this.db.prepare('SELECT COUNT(*) as count FROM api_key_usage WHERE api_key_id = ? AND timestamp > ? AND success = 0').get(apiKeyId, since);
        // Calls by day
...
export async function apiKeyAuthPlugin(fastify, options) {
    ...
    fastify.addHook('onRequest', async (request, reply) => {
        ...
        let key = request.headers[headerName.toLowerCase()];
        // Check query param if no header
        if (!key && options.queryParamName) {
            key = request.query?.[options.queryParamName];
        }
        if (!key)
        ...
        // Record usage
        service.recordUsage(apiKey.id, request.url, request.method, request.ip, request.headers['user-agent'], true);
