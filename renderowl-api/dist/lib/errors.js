...
export class ValidationError extends ApiError {
    errors;
    constructor(instance, errors, detail = 'The request contains invalid data') {
        super(ErrorTypes.VALIDATION_FAILED, 'Validation Failed', 400, detail, instance, { errors });
        this.errors = errors;
    }
}
export class NotFoundError extends ApiError {
    constructor(resourceType, resourceId, instance) {
        super(ErrorTypes.NOT_FOUND, `${resourceType} Not Found`, 404, `${resourceType} with ID "${resourceId}" does not exist`, instance);
    }
}
export class RateLimitError extends ApiError {
    constructor(result, instance) {
        super(ErrorTypes.RATE_LIMIT_EXCEEDED, 'Rate Limit Exceeded', 429, `Rate limit of ${result.limit} requests per window exceeded. Retry after ${result.retryAfter} seconds.`, instance, {
...
// ============================================================================
export function sendError(reply, status, type, detail, instance, extensions) {
    const title = type
        .split('/')
        .pop()
        ?.split('-')
...
export function sendValidationError(reply, errors, instance) {
    return reply.status(400).send({
        type: ErrorTypes.VALIDATION_FAILED,
        title: 'Validation Failed',
        status: 400,
...
export function sendNotFound(reply, resourceType, resourceId, instance) {
...
export function sendRateLimitError(reply, result, instance) {
    return reply
        .status(429)
        .headers({
