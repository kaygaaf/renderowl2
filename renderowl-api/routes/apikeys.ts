...
export default async function apiKeyRoutes(
  ...
  const apiKeyService = new ApiKeyService(
    process.env.APIKEY_DB_PATH || './data/apikeys.db'
  );
  ...
  interface CreateApiKeyBody {
    name: string;
    scopes?: string[];
    template?: keyof typeof SCOPE_TEMPLATES;
  ...
  fastify.get<{ Params: GetApiKeyParams }>('/:key_id', async (request, reply) => {
    const userId = (request.user as any)?.id;
  ...
  fastify.patch<{ Params: UpdateApiKeyParams; Body: UpdateApiKeyBody }>(
    '/:key_id',
    async (request, reply) => {
      const userId = (request.user as any)?.id;
      const { key_id } = request.params;
  ...
  fastify.delete<{ Params: RevokeApiKeyParams; Body: RevokeApiKeyBody }>(
    '/:key_id',
    async (request, reply) => {
      const userId = (request.user as any)?.id;
      const { key_id } = request.params;
  ...
  fastify.get<{ Params: GetStatsParams; Querystring: GetStatsQuery }>(
    '/:key_id/stats',
    async (request, reply) => {
      const userId = (request.user as any)?.id;
      const { key_id } = request.params;
