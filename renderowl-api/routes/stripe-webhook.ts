...
type PlanTier = keyof typeof TIER_CREDITS;
...
export default async function stripeWebhookHandler(
  ...
  try {
    ...
    const rawBody = (request.rawBody as Buffer)?.toString() || (request.body as string);
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session): Promise<void> {
  ...
  addCredits(
    userId,
    credits,
    `Purchase: ${tier} plan (${credits} credits)`,
    {
  ...
  if (session.subscription) {
    updateUserStripeInfo(userId, undefined, session.subscription as string);
  }
    ...
    if (user.stripe_subscription_id === subscriptionId) {
      ...
      if (credits) {
        addCredits(
          user.id,
          credits,
          `Monthly renewal: ${tier} plan (${credits} credits)`,
          {
