...
// Simulated presigned upload URL storage
...
const assetsStore = new Map<string, AssetRecord>();
...
const detectAssetType = (contentType: string, filename: string): AssetRecord['type'] => {
  const mimeToType: Record<string, AssetRecord['type']> = {
    'video/mp4': 'video',
    'video/webm': 'video',
  ...
  const extToType: Record<string, AssetRecord['type']> = {
    mp4: 'video',
    webm: 'video',
    mov: 'video',
    avi: 'video',
...
const generatePresignedUploadUrl = (
  ...
  filename: string,
  _contentType: string
): { uploadUrl: string; storagePath: string; expiresAt: Date } => {
...
const generateSignedDownloadUrl = (asset: AssetRecord): string => {
...
export default async function assetsRoutes(
  ...
  fastify.get<{ Params: ListAssetsParams; Querystring: ListAssetsQuery }>(
    '/',
    async (request, reply) => {
      ...
      let assets = Array.from(assetsStore.values()).filter(
  ...
  // ========================================================================
  // Create Asset Upload (Get Presigned URL)
  // ========================================================================
  ...
  fastify.post<{ Params: CreateUploadParams; Body: CreateUploadBody }>(
    '/upload',
    async (request, reply) => {
      const { project_id } = request.params;
      ...
      const { uploadUrl, storagePath, expiresAt } = generatePresignedUploadUrl(
        assetId,
        data.filename,
        data.content_type
      );
  ...
  fastify.post<{ Params: ConfirmUploadParams; Body: ConfirmUploadBody }>(
    '/:id/upload-complete',
    async (request, reply) => {
      ...
      const extractedMetadata: AssetRecord['metadata'] = {
        ...providedMetadata,
      };
  ...
  fastify.get<{ Params: GetAssetParams; Querystring: GetAssetQuery }>(
    '/:id',
    async (request, reply) => {
      const { project_id, id } = request.params;
  ...
  fastify.patch<{ Params: UpdateAssetParams; Body: UpdateAssetBody }>(
    '/:id',
    async (request, reply) => {
      const { project_id, id } = request.params;
  ...
  fastify.delete<{ Params: DeleteAssetParams }>('/:id', async (request, reply) => {
    const { project_id, id } = request.params;
  ...
  // ========================================================================
  // Get Asset Download URL (Direct endpoint)
  // ========================================================================
  ...
  fastify.get<{ Params: GetDownloadUrlParams }>('/:id/download', async (request, reply) => {
    const { project_id, id } = request.params;
