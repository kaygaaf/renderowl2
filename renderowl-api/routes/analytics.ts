...
export class AnalyticsService {
  ...
  trackEvent(params: {
    ...
    this.db.run(
      ...
      VALUES (?, ?, ?, ?, ?, ?)`,
      [event.id, event.userId, event.projectId || null, event.eventType, JSON.stringify(event.eventData), event.createdAt]
    );
  ...
  getRenderStats(userId: string, projectId?: string, fromDate?: string, toDate?: string): RenderStats {
    ...
    const result = this.db.querySingle(query, params) as any;
  ...
  getTimeSeriesData(
    userId: string,
    metric: 'renders' | 'credits' | 'frames' | 'duration',
    days: number = 30,
    projectId?: string
  ): TimeSeriesPoint[] {
  ...
  getNotifications(userId: string, options?: { unreadOnly?: boolean; limit?: number }): Notification[] {
    let query = `
    ...
    const results = this.db.query(query, params) as any[];
  ...
  getUnreadCount(userId: string): number {
    const result = this.db.querySingle(
...
export default async function analyticsRoutes(fastify: FastifyInstance, opts: any) {
  ...
  fastify.get('/summary', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { project_id, from, to } = request.query as any;
  ...
  // --------------------------------------------------------------------------
  fastify.get('/timeseries', async (request: FastifyRequest, reply: FastifyReply) => {
    const userId = (request as any).user?.id;
    if (!userId) {
      return reply.status(401).send({ error: 'Unauthorized' });
    }
    ...
    const data = service.getTimeSeriesData(userId, metric as any, parseInt(days), project_id);
  ...
  fastify.post('/events', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { event_type, project_id, event_data } = request.body as any;
  ...
  fastify.get('/notifications', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { unread_only, limit = '50' } = request.query as any;
